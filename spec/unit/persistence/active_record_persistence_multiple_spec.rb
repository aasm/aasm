        ...
        context "when AASM enum setting is not enabled and aasm column not present" do
          ...
          it "should raise an error for transitions" do
            if ActiveRecord.gem_version >= Gem::Version.new('7.2.0')
              expect{multiple_with_enum_without_column.send(:view, :left)}.to raise_error(RuntimeError, /Unknown enum attribute 'status' for MultipleWithEnumWithoutColumn/)
            elsif ActiveRecord.gem_version >= Gem::Version.new('7.1.0')
